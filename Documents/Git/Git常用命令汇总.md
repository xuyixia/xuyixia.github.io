Git是一个版本控制的工具，它有很多命令用以驱动其工作。正因为其本质上是一个工具，所以它的命令我们不必死记硬背，随用随查都行。千万不要以是否能记住命令来定义自己是否理解它，这样会加重抵触心理。

插一句题外话，课本上的哪些东西需要死记硬背，哪些不需要呢？我们可以将书本中的知识点、结论、总结归类到要背的一类中，其余的类似拓展知识、延伸、和操作指南等这类内容，归类到不必背的一类中。
